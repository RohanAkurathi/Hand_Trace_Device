...
type Props = NativeStackScreenProps<RootStackParamList, "FigureEightResults">;

const formatScore = (value: number) => `${Math.round(value)} / 100`;
const formatPercent = (value: number) => `${value.toFixed(1)}%`;
const formatPx = (value: number) => `${value.toFixed(1)} px`;

export default function FigureEightResultsScreen({ navigation, route }: Props) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Figure-8 ∞ Results</Text>
      <Text style={styles.subtitle}>
        Summary of tracing accuracy and smoothness for clinical review.
      </Text>
      ...
      <View style={styles.metricsCard}>
        ...
        <View style={styles.metricRow}>
          <Text style={styles.metricLabel}>Path Coverage</Text>
          <Text style={styles.metricValue}>
            {formatScore(metrics.scores.coverage)}{" "}
